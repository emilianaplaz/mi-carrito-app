...
const ComprarLista = () => {
  ...
  const loadRecommendations = async (groceryList: GroceryList, budget?: number) => {
    ...
    try {
      ...
      console.log(
        "List items:",
        groceryList.items.map((i) => i.name),
      );
      console.log(
        "Sample productos from DB:",
        (allPricesData || []).slice(0, 10).map((p: any) => p.producto),
      );
      ...
      ["Platano", "manzana", "tomate", "cebolla", "ajo"].forEach((itemName) => {
        ...
        console.log(
          `"${itemName}" matches:`,
          matches.length,
          matches.slice(0, 3).map((p: any) => p.producto),
        );
      });

      // Fetch ALL supermarkets to show complete breakdown
      const { data: allSupermarkets } = await supabase.from("supermarkets").select("name").order("name");
          ...
          <Alert className="mb-6 border-orange-500 bg-orange-50 dark:bg-orange-950/20 relative">
            ...
            <div className="flex-1 pr-8">
              ...
              <AlertDescription className="text-orange-700 dark:text-orange-400">
                Los siguientes productos no están disponibles en ningún supermercado:{" "}
                {itemsWithoutPrices.map((item) => item.name).join(", ")}
              </AlertDescription>
            </div>
          </Alert>
        )}
        ...
        <Card className="p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="automation-toggle">{isAutomated ? "Activado" : "Desactivado"}</Label>
            </div>
          </div>
          ...
          {isAutomated && !showAutomationSettings && nextScheduledDate && (
            <div className="pt-4 border-t">
              <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-950/20 rounded-lg border border-green-200 dark:border-green-800">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="font-medium text-green-800 dark:text-green-300">
                      Próxima programación: {format(nextScheduledDate, "PPP", { locale: es })}
                    </p>
                ...
                <Button variant="outline" size="sm" onClick={() => setShowAutomationSettings(true)}>
                  Editar
                </Button>
              </div>
            </div>
          ...
          <>
            ...
            {aiSummary && recommendations.length > 0 && (
              <div className="space-y-4 mb-6">
                ...
                {recommendations.slice(0, 2).map((rec, index) => {
                  ...
                  const displayTitle = rec.displayLabel === "Mejor Opción" ? "🎯 Mejor Opción" : "💰 Opción Más Barata";

                  return (
                    <Card
                      key={index}
                      className={`p-6 border-2 ${(rec.missingCount || 0) === 0 ? "bg-card border-primary" : "bg-card border-accent"}`}
                    >
                      <div className="flex items-start gap-3 mb-4">
                        <Sparkles
                          className={`h-7 w-7 mt-1 flex-shrink-0 ${(rec.missingCount || 0) === 0 ? "text-green-600 dark:text-green-400" : "text-orange-600 dark:text-orange-400"}`}
                        />
                        <div className="flex-1">
                          <h3
                            className={`text-xl font-bold mb-2 ${(rec.missingCount || 0) === 0 ? "text-green-800 dark:text-green-300" : "text-orange-800 dark:text-orange-300"}`}
                          >
                            {displayTitle}
                          </h3>
                          ...
                          <div
                            className={`rounded-lg p-4 border ${(rec.missingCount || 0) === 0 ? "bg-card border-primary" : "bg-card border-accent"}`}
                          >
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center gap-3">
                                <Store
                                  className={`h-8 w-8 ${(rec.missingCount || 0) === 0 ? "text-green-600 dark:text-green-400" : "text-orange-600 dark:text-orange-400"}`}
                                />
                                <div>
                              ...
                              <div className="text-right">
                                <p
                                  className={`text-3xl font-bold ${(rec.missingCount || 0) === 0 ? "text-green-600 dark:text-green-400" : "text-orange-600 dark:text-orange-400"}`}
                                >
                                  ${rec.totalPrice.toFixed(2)}
                                </p>
                                <p
                                  className={`text-xs font-semibold ${(rec.missingCount || 0) === 0 ? "text-green-700 dark:text-green-300" : "text-orange-700 dark:text-orange-300"}`}
                                >
                  ...
                  (() => {
                    ...
                    list.items.forEach((listItem) => {
                      ...
                      if (priceData && priceData.availablePrices.length > 0) {
                        ...
                        priceData.availablePrices.forEach((price: any) => {
                          if (
                            !bySupermarket.has(price.supermarket) ||
                            bySupermarket.get(price.supermarket).price > price.price
                          ) {
                    ...
                    list.items.forEach((listItem) => {
                      supermarketMap.forEach((data, supermarket) => {
                        const hasItem = data.available.some(
                          (a: any) => a.name.toLowerCase() === listItem.name.toLowerCase(),
                        );
                        if (!hasItem) {
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                {data.missing.map((prod: string, idx: number) => (
                                  <div
                                    key={idx}
                                    className="flex items-center gap-2 text-sm p-2 rounded bg-orange-50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-800"
                                  >
            ...
            {itemsWithoutPrices.length > 0 && (
              <Card className="p-6 mb-6 border-2 border-red-500/30 bg-red-50/50 dark:bg-red-950/20">
                ...
                <p className="text-sm text-muted-foreground mb-3">Estos productos no están en nuestra base de datos:</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {itemsWithoutPrices.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-2 text-sm p-2 rounded-lg bg-card border border-destructive"
                    >
